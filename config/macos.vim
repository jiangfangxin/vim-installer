...
" 1. normal模式下按F7是复制单行，如果按F7之前提供了数字，对应需要复制的行数
"     nnoremap <F7> :<C-u><C-r>=line('.')<CR>,+<C-r>=v:count-1<CR>w !pbcopy<CR><CR>
" 2. visual模式下按F7是复制选择的内容，这个新建了一个缓冲文件临时存储复制的内容，
" 等把缓冲文件内容复制到Mac剪贴板之后再删除缓冲区.
"     vnoremap <F7> "ay:tabe ~/vimclipboard<CR>"ap:w !pbcopy<CR><CR>:bdelete!<CR>
